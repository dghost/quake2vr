...
=================
*/
void R_LerpMD2Verts (int32_t nverts, dtrivertx_t *v, dtrivertx_t *ov, dtrivertx_t *verts, float *lerp, float move[3], float frontv[3], float backv[3], float normalscale)
{
	int32_t i;
...
void R_DrawAliasMD2FrameLerp (dmdl_t *paliashdr, float backlerp)
{
	...
	int32_t				*order; 
	int32_t				i, count, index_xyz, vertcount, baseindex;
	float			*lerp;
	...
	order = (int32_t *)((byte *)paliashdr + paliashdr->ofs_glcmds);
...
void R_BuildMD2ShadowVolume (dmdl_t *hdr, vec3_t light, float projectdistance, qboolean nocap)
{
	...
	ot = tris = (dtriangle_t *)((uint8_t*)hdr + hdr->ofs_tris);
...
void R_DrawAliasMD2PlanarShadow (dmdl_t *paliashdr, qboolean mirrored)
{
	...
	GLenum	mode;
	int32_t		i, count, /*va, index,*/ vertcount, baseindex;
...
static qboolean R_CullAliasMD2Model (vec3_t bbox[8], entity_t *e)
{
	...
	int32_t			p, f, mask, aggregatemask = ~0;
...
//		R_FlipModel (true, false);
	...
	shadevector[2] = 1;
	VectorNormalize (shadevector);
	switch ((int32_t)(r_shadows->value))
	{
	case 0:
		break;
	case 2:
