...
static int32_t rate_tbl[] = { 2500, 3200, 5000, 10000, 25000, 0 };
...
static qboolean PlayerConfig_ScanDirectories (void)
{
    ...
    if ( (dirnames = FS_ListFilesWithPaks("players/*", &ndirs, SFF_SUBDIR, 0)) != 0 )
    {
        ...
        for (i = 0; i < npms; i++)
        {
            ...
            skinnames = Z_TagMalloc(sizeof(char *) * (nskins+1), ZONE_MENU);
            ...
            if (nimagefiles)
                for (s = 0, k = 0; k < nimagefiles-1; k++)
                {
                    ...
                    if ( IsValidSkin(imagenames, nimagefiles, k) )
                    {
                        ...
                        skinnames[s] = Z_TagStrdup(scratch, ZONE_MENU);
                        s++;
                    }
                }
...
qboolean PlayerConfig_CheckIncerement (int32_t dir, float x, float y, float w, float h)
{
