...
static void allocateNameArrays(int numNames) {
	...
	num_names = numNames;
	shortnames = (const char **) Z_TagMalloc(size, TAG_MENU);
	longnames = (const char **) Z_TagMalloc(size, TAG_MENU);
}
...
static void loadModNames() {
	...
	Q_SSetInit(&mod_name_storage, dirs.currentSize, 24, TAG_MENU);
	Q_SSetInit(&mod_title_storage, dirs.currentSize, 24, TAG_MENU);
	...
	Q_SSetGetStrings(&mod_name_storage, shortnames, num_names);
	Q_SSetGetStrings(&mod_title_storage, longnames, num_names);
	shortnames[num_names] = NULL;
	longnames[num_names] = NULL;
	Q_SSetFree(&dirs);

	currentGame = -1;
	for (i = 0; i < num_names; i++) {
		Com_Printf("Comparing %s and %s\n", currentGameName, shortnames[i]);
		if (!Q_strcasecmp(currentGameName, shortnames[i])) {
			currentGame = i;
			//break;
		}
	}
	...
	s_game_mod_choice.numitemnames = num_names;
}
...
static void refreshModNames() {
	...
	//        Com_Printf("Comparing %s and %s\n", currentGameOption, shortnames[i]);
	        if (!Q_strcasecmp(currentGameOption, shortnames[i])) {
