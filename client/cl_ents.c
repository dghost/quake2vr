...
void CL_ParseDelta (entity_state_t *from, entity_state_t *to, int32_t number, int32_t bits)
{
	...
	*to = *from;
	...
	else //new CL_ParseDelta code
	{
		...
		int32_t ignore;	// holder for messages to be ignored
	#endif
...
void CL_ParsePlayerstate (frame_t *oldframe, frame_t *newframe)
{
	...
	else //new CL_ParsePlayerstate code
	{
		...
		//Knightmare 4/5/2002- read as int16_t
		if (flags & PS_WEAPONINDEX)
...
void CL_AddViewWeapon (player_state_t *ps, player_state_t *ops)
{
	...
	if (gun.model)
	{
		...
		{
			int32_t oldeffects = gun.flags, pnum;
			entity_state_t	*s1;
	...
	if (gun2.model)
	{
		...
		if (cl_weapon_shells->value)
		{
			int32_t oldeffects = gun2.flags, pnum;
			entity_state_t	*s1;
