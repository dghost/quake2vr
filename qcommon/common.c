...
{
	int32_t		i;
    hash32_t hash = Hash32(parm, strlen(parm));
	for (i=1 ; i<com_argc ; i++)
	{
		if (!HashEquals32(hash, com_argv_hash[i]) && !strcmp (parm,com_argv[i]))
			return i;
	}
...
void COM_InitArgv (int32_t argc, char **argv)
{
	...
	for (i=0 ; i<argc ; i++)
	{
		if (!argv[i] || strlen(argv[i]) >= MAX_TOKEN_CHARS )
			...
			com_argv[i] = argv[i];
            com_argv_hash[i] = Hash32(argv[i], strlen(argv[i]));
        }
	}
}
...
void COM_AddParm (char *parm)
{
    ...
    com_argv_hash[com_argc++] = Hash32(parm, strlen(parm));
}
